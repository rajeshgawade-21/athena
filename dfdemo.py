PROJECT='folkloric-alpha-314903'
BUCKET='athena_staging_bucket'
REGION='us-west1'

from datetime import datetime
import apache_beam as beam
import os
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "C:/Rajesh/python/folkloric-alpha-314903-7921ecdf6264.json"
...
def run():
    ...
    """
    (p 
     | 'bigquery' >> beam.io.Read(beam.io.ReadFromBigQuery(
    ...
    (p 
     | 'bigquery' >> beam.io.Read(beam.io.ReadFromBigQuery(
         query="SELECT text FROM `folkloric-alpha-314903.athena_her_hack_21_rsg.full-twitter-data` WHERE LENGTH(text)>5",
         use_standard_sql=True))
      | 'txt'      >> beam.Map(lambda x : x['text'])
      | 'doc'      >> beam.Map(lambda x : nlp.Document(x, type='PLAIN_TEXT'))
    #  | 'todict'   >> beam.Map(lambda x : nlp.Document.to_dict(x))
      | 'nlp'      >> nlp.AnnotateText(features, timeout=10)
      | 'parse'    >> beam.Map(parse_nlp_result)
      | 'gcs'      >> beam.io.WriteToText('gs://{}/output_new.txt'.format(BUCKET), num_shards=1)
    )
    result = p.run()
